...
#         train_loss = lambda pred, gt: -(gt * torch.log(pred + 1e-8) + (1 - gt.bool().float()) * torch.log(1 - pred + 1e-8)).mean()
    ...
    replacing_table = list(np.random.randint(int(data_seq_len*replacing_rate[0]), int(data_seq_len*replacing_rate[1]), size=1000))#size=10000
    replacing_table_index = 0
    replacing_table_length = 1000 #10000
    ...
    for i in range(options.initial_iter, max_iters):
        ...
        x_rep = []  # 替换的区间列表
        x_anomaly = torch.zeros(n_batch, data_seq_len, device=device)  # 异常点列表
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_steps", default=50, type=int, help='maximum_training_steps')
    parser.add_argument("--summary_steps", default=5, type=int, help='steps for summarizing and saving of training log')
    ...
    parser.add_argument("--dataset", default='DTL', type=str, help='SMAP/MSL/SMD/SWaT/WADI')
    ...
    parser.add_argument("--window_sliding", default=64, type=int, help='sliding steps of windows for validation')
